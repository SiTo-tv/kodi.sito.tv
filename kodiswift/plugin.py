...
class Plugin(XBMCMixin):
    ...
    def __init__(self, name=None, addon_id=None, plugin_file=None,
        ...
        if not self._info_type:
            ...
            self._info_type = 'video'
            if self._addon_id:
                id_parts = self._addon_id.split('.')
                if len(id_parts) >= 2:
                    self._info_type = types.get(id_parts[1], 'video')
    ...
    def run(self):
        ...
        log.info('Handling incoming request for %s', self.request.path)
    ...
    def _parse_request(url=None, handle=None):
        ...
        if handle is None:
            handle = sys.argv[1]
        log.info('request url %s', sys.argv)
